...
static Logger logger = Logger.getLogger("PulseReceiverAppl");
	...
	public static void main(String[] args) throws Exception{
		...
		request = PutItemRequest.builder()
				.tableName(configValues.getTableName());
	...
	private static void processReceivedData(DatagramPacket packet) {
		...
		logger.finer(String.format("table: %s added item with partition key is %d,"
				+ " sorted key is %d", configValues.getTableName(), sensorData.patientId(), sensorData.timestamp()));
	...
	private static void logAbnormalValue(SensorData sensorData) {
		...
		if ( pulseValue > configValues.getWarnMaxPulse()
				&& pulseValue <= configValues.getMaxThresholdPulse()) {
			logger.warning(String.format("pulse value greater than %d, sensor data %s",
					configValues.getWarnMaxPulse(), sensorData));
		} else if (pulseValue < configValues.getWarnMinPulse()
				&& pulseValue >= configValues.getMinThresholdPulse()) {
			logger.warning(String.format("pulse value less than %d, sensor data %s",
					configValues.getWarnMinPulse(), sensorData));
		} else if (pulseValue > configValues.getMaxThresholdPulse()) {
			...
			logger.severe(String.format("pulse value less than %d, sensor data %s",
					configValues.getMinThresholdPulse(), sensorData));
		}
